...
subroutine recqmoy(recok,cm,cc,cr,phi,th,u,tau,irec,t,dtrec,vmax)
...
real*8, dimension(cc%n) :: phi,th,u,tau
     ...
     if (rang .eq. 0) then
            ...
            !write(60,rec=irec) real(sum(exp(phi))*cm%vb0/cc%n,8)
            ...
            statusnetcdf=nf90_put_var(cr%ncqmoyid, cr%qmoy_varid(3), sum(exp(phi))*cm%vb0/cc%n, start=(/irec/))
