...
program fault_3d_infperiodic_slip_pnl
...
implicit none
integer :: iter,i
integer :: ii_inj,jj_inj,i_inj
integer :: irec,irecloc,irecp,irecphvc,irecevt,recok,reclocok,recpok,recphvcok,isevt
integer :: nelevt_proc,nelevt_proc0
integer, dimension(4) :: iterex
integer, dimension(:), allocatable :: nelevt,vrecpoints
real*8 :: t,dt,dt0,dtrec,dtrecloc,dtrecp,dtrecphvc
real*8 :: start
...
real*8, dimension(:), allocatable :: phi,th,u,tau,p
...
real*8, dimension(:), allocatable :: p_proc
...
!--Parameters-------------------------------------------------------------------------------------------------------!
!-------------------------------------------------------------------------------------------------------------------!
 call param_fault_rns_p(cm,cc,cr,chy)
...
if (cr%catal .eq. 1) then
 ...
 if (rang .eq. 0) then
    allocate(phi(cc%n),th(cc%n),u(cc%n),tau(cc%n),p(cc%n))
...
!-------------------------------------------------------------------------------------------------------------------!
 call init_rns_3d_rg_p(a_proc,b_proc,dc_proc,s_proc,phi_proc,th_proc,u_proc,p_proc,cm,cc,cr)
 t=0.0
 tex(1)=0.0
 tex(2)=0.0
 tex(3)=0.0
 ...
 call normalize_rns_p(a_proc,b_proc,dc_proc,s_proc,u_proc,phi_proc,th_proc,p_proc,cm,chy,cc,cr)
  ...
  call MPI_BARRIER(MPI_COMM_WORLD,code)     
   !---------------------------------------------------------------------------------------------!
   !--Find index of injection point------------------------------------------------------------!
   !---------------------------------------------------------------------------------------------!
 ii_inj=1+floor(real(chy%paraminj(2)/cc%dx+0.5*cc%nx))
 jj_inj=1+floor(real(chy%paraminj(3)/cc%dy+0.5*cc%ny))
 i_inj=(ii_inj-1)*cc%ny+jj_inj
 chy%rang_inj=floor(real((i_inj-1)/cc%nvalpr))
 chy%i_inj_proc=i_inj-chy%rang_inj*cc%nvalpr          
!-------------------------------------------------------------------------------------------------------------------!
...
do iter=1,cc%niter
  ...
  if ((recok .eq. 1) .or. (recpok .eq. 1) .or. (reclocok .eq. 1) .or. (recphvcok .eq. 1)) then
     ...
     do i=1,cc%nvalpr
        tau_proc(i)=(cm%mu0+cm%r*a_proc(i)*phi_proc(i)+b_proc(i)*th_proc(i))*(s_proc(i)-p_proc(i))
     end do
     ...
     call MPI_GATHER(phi_proc,cc%nvalpr,MPI_DOUBLE_PRECISION,phi,cc%nvalpr,MPI_DOUBLE_PRECISION,0,&
      ...
      call MPI_GATHER(th_proc,cc%nvalpr,MPI_DOUBLE_PRECISION,th,cc%nvalpr,MPI_DOUBLE_PRECISION,0,&
                   MPI_COMM_WORLD,code)
      call MPI_GATHER(p_proc,cc%nvalpr,MPI_DOUBLE_PRECISION,p,cc%nvalpr,MPI_DOUBLE_PRECISION,0,&
                   MPI_COMM_WORLD,code)
  ...
  if (reclocok .eq. 1) then
      call recqloc_p(reclocok,vrecpoints,cm,chy,cc,cr,phi,th,u,p,tau,irecloc,t,dtrecloc)
   end if
  !-------------------------------------------------------------------------------------------------!
  !--Record profiles--------------------------------------------------------------------------------!
  !-------------------------------------------------------------------------------------------------!
  if (recpok .eq. 1) then
     call recprofils_p(recpok,cm,chy,cr,cc,phi,th,u,tau,p,irecp,t,dtrecp)
  end if
  !-------------------------------------------------------------------------------------------------!
  !--Record central vertical and horizontal profiles------------------------------------------------!
  !-------------------------------------------------------------------------------------------------!
  if (recphvcok .eq. 1) then
     call rechvcprofils_p(recphvcok,cm,cr,cc,phi,th,u,tau,p,irecphvc,t,dtrecphvc)
  end if
  !--------------------------------------------------------------------------------------------------------!
  !--Catalogue Record--------------------------------------------------------------------------------------!
  !--------------------------------------------------------------------------------------------------------!
  if (cr%catal .eq. 1) then   
      call reccatal_3d_rg_p(cc,cm,cr,phi_proc,th_proc,tau_proc,u_proc,s_proc,p_proc,a_proc,b_proc,irecevt,&
      			 t,vmax,isevt,t0evt,dt0evt,dtevt,m0_proc,&
...
call rk4f_3d_infperiodic_slip_pnl(t,phi_proc,th_proc,kappa_proc,&
...
end program fault_3d_infperiodic_slip_pnl

include 'param_fault_rns_p.f90'
include 'init_rns_3d_rg_p.f90'
include 'renewoutfiles.f90'
include 'normalize_rns_p.f90'
include 'tbp_rns_3d_infperiodic.f90'
include 'ci2xy.f90'
include 'xy2ci.f90'
include 'frns_slip_p.f90'
include 'grns_slip_p.f90'
include 'elastic_kernel_3d_infperiodic.f90'
include 'plan_static_stress_periodic.f90'
include 'rk4f_3d_infperiodic_slip_pnl.f90'
include 'varecok_3d.f90'
include 'recqmoy.f90'
include 'recqloc_p.f90'
include 'recprofils_p.f90'
include 'rechvcprofils_p.f90'
include 'reccatal_3d_rg_p.f90'
include 'diffu.f90'
include 'transfer_end_points_2d.f90'
include 'pressrate_nl2d.f90'
