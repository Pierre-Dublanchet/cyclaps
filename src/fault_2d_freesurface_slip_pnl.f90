...
program fault_2d_freesurface_slip_pnl
...
real*8, dimension(:,:),allocatable :: Kstatic_proc
type(pmeca) :: cm
type(phydro) :: chy
type(pcomput) :: cc
type(prec) :: cr
...
allocate(Kstatic_proc(cc%nvalpr,cc%n))
...
!-------------------------------------------------------------------------------------------------------------------!
 ...
 call normalize_rns_p(a_proc,b_proc,dc_proc,s_proc,u_proc,phi_proc,th_proc,p_proc,cm,chy,cc,cr)
   ...
   !--Compute modes of elastic kernel------------------------------------------------------------!
      !------------------------------------------------------------------------------------------------------!
 call elastic_kernel_2d_freesurface(Kstatic_proc,cm,cc)
 call MPI_BARRIER(MPI_COMM_WORLD,code)               
  !------------------------------------------------------------------------------------------------------!
 !--Compute (constant) external stressing-----------------------------!
  !------------------------------------------------------------------------------------------------------!
 do i=1,cc%nvalpr
 	tbp_proc(i)=cm%paramdsext(1)
 end do    
 !---------------------------------------------------------------------------------------------!
...
do iter=1,cc%niter
 ...
 call rk4f_2d_freesurface_slip_pnl(t,phi_proc,th_proc,tbp_proc,Kstatic_proc,&
...
end program fault_2d_freesurface_slip_pnl
...
include 'elastic_kernel_2d_freesurface.f90'
include 'lin_static_freesurface.f90'
include 'rk4f_2d_freesurface_slip_pnl.f90'
