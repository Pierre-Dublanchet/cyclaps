...
subroutine reccatal_3d_rg_seas(cc,cr,cm,phi_proc,th_proc,tau_proc,u_proc,s_proc,a_proc,b_proc,irecevt,t,vmax,isevt,&
...
real*8, dimension(cc%nvalpr) :: trupture_proc
real*8, dimension(cc%n) :: trupture
     ...
     if ((vmax .gt. cm%vsis) .and. (isevt .eq. 0)) then
        ...
        do i=1,cc%nvalpr
           if (exp(phi_proc(i)) .gt. cm%vsis) then 
              ...
              trupture_proc(i)=1e9*cm%vb0/cm%dc0
           end if
        end do
      ...
      if ((isevt .eq. 1) .and. (vmax .lt. cm%vsis)) then
        ...
        do i=1,cc%nvalpr
         ...
         call MPI_GATHER(tauf_proc,1,MPI_DOUBLE_PRECISION,tauf,1,MPI_DOUBLE_PRECISION,0,&
                      MPI_COMM_WORLD,code)
         call MPI_GATHER(trupture_proc,cc%nvalpr,MPI_DOUBLE_PRECISION,trupture,cc%nvalpr,MPI_DOUBLE_PRECISION,0,&
                      MPI_COMM_WORLD,code)
         ...
         if (rang .eq. 0) then
            ...
            statusnetcdf=nf90_put_var(cr%nceqkcatid, cr%eqkcat_varid(12), sum(taui)/sum(nelevt), start=(/irecevt/))
            statusnetcdf=nf90_put_var(cr%nceqkcatid, cr%eqkcat_varid(13), sum(tauf)/sum(nelevt), start=(/irecevt/))

            print*,' End of event, duration (s): ',tev,' magnitude: ',(2.0/3.0)*log10(sum(m))-6.0

            !--Write rupture times
            if (irecevt .eq. 1) then
               statusnetcdf=nf90_create(trim(cr%path_res)//'rupture_times_eqk1.nc', NF90_CLASSIC_MODEL, ncid)
               statusnetcdf=nf90_def_dim(ncid, "index",cc%n,x_dimid)
               statusnetcdf=nf90_def_var(ncid, "rupture times", NF90_DOUBLE,[x_dimid],varid)
               statusnetcdf=nf90_enddef(ncid)
               statusnetcdf=nf90_put_var(ncid, varid, trupture(:)*cm%dc0/cm%vb0)
               statusnetcdf=nf90_close(ncid)
               !open(91,file=trim(cr%path_res)//'rupture_times_eqk1.data',form='unformatted',access='direct',recl=8)
               !do i=1,cc%n
               !   write(91,rec=i) real(trupture(i)*cm%dc0/cm%vb0,8)
               !end do
               !close(91)
            end if
...
end subroutine reccatal_3d_rg_seas
