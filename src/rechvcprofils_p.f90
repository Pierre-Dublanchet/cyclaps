...
subroutine rechvcprofils_p(recphvcok,cm,cr,cc,phi,th,u,tau,p,irecphvc,t,dtrecphvc)
...
real*8 :: t,dtrecphvc
real*8, dimension(cc%n) :: phi,th,u,tau,p
 character(len=10) :: numf
     ...
     if (rang .eq. 0) then
            ...
            !open(805,file=trim(cr%path_res)//'px'//trim(numf)//'.data',form='unformatted',access='direct',recl=8)
            !do i=1,cc%nx
            ...
            !   write(805,rec=i) real(0.5*(p(k)+p(l))*cm%sig0,8)
            !end do
            !close(801)
            !close(802)
            !close(803)
            ...
            statusnetcdf=nf90_def_var(ncid, "time", NF90_DOUBLE,[t_dimid],varid(6))
            statusnetcdf=nf90_def_var(ncid, "time delai", NF90_DOUBLE,[t_dimid],varid(7))
            ...
            do i=1,cc%nx
                ...
                statusnetcdf=nf90_put_var(ncid, varid(5), 0.5*(p(k)+p(l))*cm%sig0, start=(/i/))

            end do
            statusnetcdf=nf90_put_var(ncid, varid(6), t*cm%dc0/cm%vb0)
            statusnetcdf=nf90_put_var(ncid, varid(7), dtrecphvc*cm%dc0/cm%vb0)
            ...
            !open(805,file=trim(cr%path_res)//'py'//trim(numf)//'.data',form='unformatted',access='direct',recl=8)
            !do i=1,cc%ny
            !   k=(cc%nx/2-2)*cc%ny+i
            !   l=(cc%nx/2-1)*cc%ny+i
            !   write(801,rec=i) real(exp(0.5*(phi(k)+phi(l)))*cm%vb0,8)
...
end subroutine rechvcprofils_p
