...
subroutine reccatal_2d_rg_p(cc,cm,cr,phi_proc,th_proc,tau_proc,u_proc,s_proc,p_proc,a_proc,b_proc,irecevt,t,vmax,isevt,&
...
real*8 :: t0evt,dtevt,dt0evt,tev,m0_proc,m_proc,du_proc,dtau_proc,t,vmax
real*8 :: x0,y0,x,y,xi,yi
real*8, dimension(nprocs) :: m,du,dtau,xa,ya
real*8, dimension(cc%nvalpr) :: phi_proc,th_proc,tau_proc,u_proc,s_proc,p_proc,a_proc,b_proc,xa_proc,ya_proc,u0_proc,tau0_proc
     ...
     nelevt_proc=0  !-number of elements participating in the eqk rupture
        do i=1,cc%nvalpr
           if (exp(phi_proc(i)) .gt. cm%vsis) then 
              ...
              ya_proc(i)=0.0
              u0_proc(i)=u_proc(i)
              tau0_proc(i)=(cm%mu0+cm%r*a_proc(i)*phi_proc(i)+b_proc(i)*th_proc(i))*(s_proc(i)-p_proc(i))
           end if
        end do
      ...
      if ((isevt .eq. 1) .and. (vmax .lt. cm%vsis)) then
		...
		m_proc=sum(u_proc(:))*cc%dx-m0_proc
        ...
        do i=1,cc%nvalpr
           if (u0_proc(i) .gt. 0.0) then
              ...
              dtau_proc=dtau_proc+(cm%mu0+cm%r*a_proc(i)*phi_proc(i)+b_proc(i)*th_proc(i))*(s_proc(i)-p_proc(i))-tau0_proc(i)
           end if
        end do
         ...
         !--Initiation time-duration-time since last eqk-x initiation-y initiation-x baricenter-y baricenter-number of points involved-       coseismic moment-average stress drop-average coseismic slip--! 
         if (rang .eq. 0) then
         !-Catalogue entries with physical units----!
           ...
           do i=1,nprocs
           	...
           	m(i)=m(i)*(cm%mu**2)*(cm%dc0**2)/((cm%b0)*(cm%sig0))
           ...
           !--Write catalogue entries
            ...
            !                  real(sum(m),8),real(sum(dtau)/sum(nelevt),8),real(sum(du)/sum(nelevt),8)
            !call flush(90)
...
end subroutine reccatal_2d_rg_p
