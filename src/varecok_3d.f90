...
subroutine varecok_3d(cr,iter,iterex,vmax,vmaxex,t,tex,dtrec,dtrecp,dtrecloc,dtrecphvc,recok,reclocok,recpok,recphvcok)

use mod_cst_fault_rns
implicit none
integer :: iter,recok,recpok,reclocok,recphvcok
integer, dimension(4) :: iterex
real*8 :: t,vmax,dtrec,dtrecp,dtrecloc,dtrecphvc
...
if (cr%qmoy .eq. 1) then
  ...
  if (cr%profilhvc .eq. 1) then
     ...
     if (cr%vfrec(4) .gt. 0.0) then
        !if ((vmax .ge. vmaxex(4)*cr%vfrec(4)) .or. (vmax .le. vmaxex(4)/cr%vfrec(4))  .or. (dtrecphvc .ge. cr%dtfrec(4))) then
        if ((vmax .ge. vmaxex(4)*cr%vfrec(4)) .or. (vmax .le. vmaxex(4)/cr%vfrec(4))) then
           recphvcok=1
           vmaxex(4)=vmax
        end if
     end if
     if (cr%tfrec(4) .gt. 0.0) then
        if (t .ge. tex(4)*cr%tfrec(4)) then
           recphvcok=1
           tex(4)=t
        end if
     end if
     if ((cr%dtfrec(4) .gt. 0.0))then
        if (dtrecphvc .ge. cr%dtfrec(4)) then
           recphvcok=1
        end if
     end if
     if (cr%nitrec(4) .gt. 0.0) then
        if (iter-iterex(4) .ge. cr%nitrec(4)) then
           recphvcok=1
           iterex(4)=iter
        end if
     end if
