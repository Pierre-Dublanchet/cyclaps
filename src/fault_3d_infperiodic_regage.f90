...
program fault_3d_infperiodic_regage

use mod_cst_fault_rns
use netcdf
implicit none
integer :: iter,i
integer :: irec,irecloc,irecp,irecphvc,irecevt,recpok,recphvcok,recok,reclocok,isevt
integer :: nelevt_proc,nelevt_proc0
integer, dimension(4) :: iterex
integer, dimension(:), allocatable :: nelevt,vrecpoints
integer, dimension(:), allocatable :: vboxm_proc,vwbox_proc
integer, dimension(:), allocatable :: vboxm,vwbox
real*8 :: t,dt,dt0,dtrec,dtrecp,dtrecloc,dtrecphvc
real*8 :: start
...
real*8, dimension(:), allocatable :: trupture_proc
real*8, dimension(:), allocatable :: trupture
real*8, dimension(:,:),allocatable :: kappa_proc
...
allocate(vboxm_proc(cc%nvalpr),vwbox_proc(cc%nvalpr))
allocate(trupture_proc(cc%nvalpr))
allocate(kappa_proc(cc%ny,cc%nvalpr/cc%ny))
...
if (cr%catal .eq. 1) then
 ...
 if (rang .eq. 0) then
    allocate(phi(cc%n),th(cc%n),u(cc%n),tau(cc%n),vboxm(cc%n),vwbox(cc%n),trupture(cc%n))
...
if (cr%qloc .eq. 1) then 
   do i=1,cr%nploc
      call xy2ci(vrecpoints(i),cc,cr%xr(i),cr%yr(i))
   end do
end if

call boxmonitor(cc,cr,vboxm_proc,vwbox_proc,a_proc,b_proc)
call MPI_GATHER(vboxm_proc,cc%nvalpr,MPI_INTEGER,vboxm,cc%nvalpr,MPI_INTEGER,0,&
                     MPI_COMM_WORLD,code)
call MPI_GATHER(vwbox_proc,cc%nvalpr,MPI_INTEGER,vwbox,cc%nvalpr,MPI_INTEGER,0,&
                     MPI_COMM_WORLD,code)
...
!-------------------------------------------------------------------------------------------------------------------!
 call init_rns_3d_rg(a_proc,b_proc,dc_proc,s_proc,phi_proc,th_proc,u_proc,cm,cc,cr)
 t=0.0
 tex(1)=0.0
 tex(2)=0.0
 tex(3)=0.0
 ...
 call normalize_rns(a_proc,b_proc,dc_proc,s_proc,u_proc,phi_proc,th_proc,cm,cc,cr)
  ...
  !------------------------------------------------------------------------------------------------------!
  call elastic_kernel_3d_infperiodic(kappa_proc,cc,cm)
...
do iter=1,cc%niter
  ...
  !------------------------------------------------------------------------------------------!
   vmax_proc=maxval(exp(phi_proc(:))*vboxm_proc(:))
  ...
  call varecok_3d(cr,iter,iterex,vmax,vmaxex,t,tex,dtrec,dtrecp,dtrecloc,dtrecphvc,recok,reclocok,recpok,recphvcok)
  !---------------------------------------------------------------------------------------------------------!
  !--Gather Variables for recording-------------------------------------------------------------------------!
  !---------------------------------------------------------------------------------------------------------!
  if ((recok .eq. 1) .or. (recpok .eq. 1) .or. (reclocok .eq. 1) .or. (recphvcok .eq. 1)) then

     !--compute frictional shear stress--!
     do i=1,cc%nvalpr
        !F=cm%mu0+cm%r*a_proc(i)*phi_proc(i)+b_proc(i)*th_proc(i)-b_proc(i)*log(dc_proc(i))
        !fric=cm%r*a_proc(i)*log(0.5*exp(F/(cm%r*a_proc(i)))+sqrt(1+0.25*exp(2*F/(cm%r*a_proc(i)))))
        !tau_proc(i)=fric*s_proc(i)

        F=cm%r*a_proc(i)*phi_proc(i)+b_proc(i)*th_proc(i)-b_proc(i)*log(dc_proc(i))
        fric=cm%r*a_proc(i)*(log(0.5*exp(F/(cm%r*a_proc(i)))+sqrt(exp(-2*cm%mu0)+0.25*exp(2*F/(cm%r*a_proc(i)))))+cm%mu0)
        tau_proc(i)=fric*s_proc(i)
     end do
  ...
  !---------------------------------------------------------------------------------------------------------!
  if (recok .eq. 1) then
     call recqmoy_seas(recok,cm,cc,cr,phi,th,u,tau,irec,t,dtrec,vmax,vboxm,vwbox)
  end if
  !---------------------------------------------------------------------------------------------------------!
  !--Record local values------------------------------------------------------------------------------------!
  !---------------------------------------------------------------------------------------------------------!
  ...
  !-------------------------------------------------------------------------------------------------!
  !--Record central vertical and horizontal profiles------------------------------------------------!
  !-------------------------------------------------------------------------------------------------!
  if (recphvcok .eq. 1) then
     call rechvcprofils(recphvcok,cm,cr,cc,phi,th,u,tau,irecphvc,t,dtrecphvc)
  end if
  !--------------------------------------------------------------------------------------------------------!
  !--Catalogue Record--------------------------------------------------------------------------------------!
  !--------------------------------------------------------------------------------------------------------!
  if (cr%catal .eq. 1) then   
      call reccatal_3d_rg_seas(cc,cr,cm,phi_proc,th_proc,tau_proc,u_proc,s_proc,a_proc,b_proc,irecevt,t,vmax,isevt,&
             t0evt,dt0evt,dtevt,m0_proc,&
                    m_proc,xa_proc,ya_proc,u0_proc,tau0_proc,du_proc,dtau_proc,nelevt_proc,nelevt_proc0,&
                    trupture_proc,trupture,&
...
!--The next commented lines are used to reduce time step in case of high frequency shear stress perturbation (see nucleation of first event in SCEC SEAS benchmark BP7)
 !if ((t .le. cm%vb0/cm%dc0) .and. (dt .ge. 0.1*cm%vb0/cm%dc0)) then
 !   !print*,iter,t
 !   dt=0.02*cm%vb0/cm%dc0
 !end if


call rk4f_3d_infperiodic_regage(phi_proc,th_proc,kappa_proc,&
                  a_proc,b_proc,dc_proc,s_proc,cc,cm,dt,dt0,t)

  t=t+dt0
  dtrec=dtrec+dt0
  dtrecp=dtrecp+dt0
  dtrecloc=dtrecloc+dt0
  dtrecphvc=dtrecphvc+dt0
  dtevt=dtevt+dt0
...
end program fault_3d_infperiodic_regage

include 'param_fault_rns.f90'
include 'boxmonitor.f90'
include 'init_rns_3d_rg.f90'
include 'renewoutfiles.f90'
include 'normalize_rns.f90'
include 'tbp_rns_3d_infperiodic_dssext.f90'
!include 'dssext_rns_3d.f90'
include 'ci2xy.f90'
include 'xy2ci.f90'
include 'frns_regage.f90'
include 'grns_age.f90'
include 'elastic_kernel_3d_infperiodic.f90'
include 'plan_static_stress_periodic.f90'
include 'rk4f_3d_infperiodic_regage.f90'
include 'varecok_3d.f90'
include 'recqmoy_seas.f90'
include 'recqloc.f90'
include 'recprofils.f90'
include 'rechvcprofils.f90'
include 'reccatal_3d_rg_seas.f90'
