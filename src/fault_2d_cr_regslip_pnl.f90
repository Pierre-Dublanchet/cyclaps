...
program fault_2d_cr_regslip_pnl

use mod_cst_fault_rns
use mod_cst_fault_rns_p
use netcdf
implicit none
integer :: iter,i,k,i_inj
...
real*8, dimension(:), allocatable :: phi,th,u,tau,p,qdarcy
...
real*8, dimension(:), allocatable :: p_proc,qdarcy_proc
...
allocate(p_proc(cc%nvalpr),qdarcy_proc(cc%nvalpr))
...
allocate(rkr(nprocs),rks(nprocs),ad(nprocs))
 ...
 if (rang .eq. 0) then
    allocate(phi(cc%n),th(cc%n),u(cc%n),tau(cc%n),p(cc%n),qdarcy(cc%n))
...
!-------------------------------------------------------------------------------------------------------------------!
 call init_rns_2d_rg_p_seas(a_proc,b_proc,dc_proc,s_proc,phi_proc,th_proc,u_proc,p_proc,qdarcy_proc,cm,cc,cr)
 t=0.0
 tex(1)=0.0
 tex(2)=0.0
 tex(3)=0.0
...
do iter=1,cc%niter
 ...
 !---------------------------------------------------------------------------------------------------------!
  ...
  if ((recok .eq. 1) .or. (recpok .eq. 1)) then
     ...
     call transfer_end_points_p_1d(p_proc,p0,pe,cc)
     ...
     do i=1,cc%nvalpr
        ...
        tau_proc(i)=fric*(s_proc(i)-p_proc(i))!+dssext_proc(i)


        !print*,F,fric,tau_proc(i)

        !--compute Darcy flux---!
        call darcyvel(p_proc,p0,pe,qdarcy_proc,cc,chy,cm)
     end do
     ...
     call MPI_GATHER(p_proc,cc%nvalpr,MPI_DOUBLE_PRECISION,p,cc%nvalpr,MPI_DOUBLE_PRECISION,0,&
                      MPI_COMM_WORLD,code)
     call MPI_GATHER(qdarcy_proc,cc%nvalpr,MPI_DOUBLE_PRECISION,qdarcy,cc%nvalpr,MPI_DOUBLE_PRECISION,0,&
                      MPI_COMM_WORLD,code)
  ...
  if (reclocok .eq. 1) then
      call recqloc_p_seas(reclocok,vrecpoints,cm,chy,cc,cr,phi,th,u,p,qdarcy,tau,irecloc,t,dtrecloc)
   end if
  !-------------------------------------------------------------------------------------------------!
  !--Record profiles--------------------------------------------------------------------------------!
  !-------------------------------------------------------------------------------------------------!
  if (recpok .eq. 1) then
     call recprofils_p_seas(recpok,cm,chy,cr,cc,phi,th,u,tau,p,qdarcy,irecp,t,dtrecp)
  end if
  !--------------------------------------------------------------------------------------------------------!
  !--Catalogue Record--------------------------------------------------------------------------------------!
  !--------------------------------------------------------------------------------------------------------!
  if (cr%catal .eq. 1) then   
      call reccatal_2d_rg_p(cc,cm,cr,phi_proc,th_proc,tau_proc,u_proc,s_proc,p_proc,a_proc,b_proc,irecevt,t,vmax,isevt,&
 ...
 call rk4f_2d_cr_regslip_pnl(t,phi_proc,th_proc,kappa_proc,v0_proc,rkr,rks,ad,&
...
end program fault_2d_cr_regslip_pnl

include 'param_fault_rns_p.f90'
include 'init_rns_2d_rg_p_seas.f90'
...
include 'frns_regslip_p.f90'
include 'grns_slip_p.f90'
include 'elastic_kernel_2d_cr.f90'
include 'lin_static_stress_cr.f90'
include 'rk4f_2d_cr_regslip_pnl.f90'
include 'varecok_2d.f90'
include 'recqmoy.f90'
include 'recqloc_p_seas.f90'
include 'recprofils_p_seas.f90'
include 'reccatal_2d_rg_p.f90'
include 'diffu.f90'
include 'darcyvel.f90'
include 'transfer_end_points_1d.f90'
include 'transfer_end_points_p_1d.f90'
include 'pressrate_nl1d.f90'
